/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   new_get_next_line_utils.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tookuyam <tookuyam@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/21 19:20:35 by tookuyam          #+#    #+#             */
/*   Updated: 2024/03/21 19:20:35 by tookuyam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "new_get_next_line.h"
#include <unistd.h>
#include <stdlib.h>

static char *ft_gnl_cut(t_gnl_mem *mem);
static void *ft_memmove(void *dest, const void *src, size_t n);
static int read_line(int fd, t_gnl_mem *mem);
static int reallocate_for_read(t_gnl_mem *mem);

char *get_next_line_fd(int fd)
{
	static t_gnl_mem	mem;
	ssize_t				result;
	char				*line;

	if (mem.line_cnt == 0)
		result = 1;
	while (result != 0 && mem.line_cnt == 0)
		result = read_line(fd, &mem);
	if (result == -1)
	{
		free(mem.data);
		mem = (t_gnl_mem){ 0 };
		return (NULL);
	}
	line = ft_gnl_cut(&mem);
	if (line == NULL)
	{
		free(mem.data);
		mem = (t_gnl_mem){ 0 };
		return (NULL);
	}
	return (line);
}

char *ft_gnl_cut(t_gnl_mem *mem)
{
	char	*line;
	char	*line_ptr;
	size_t	line_index;

	if (mem->size == 0)
		return (NULL);
	line_ptr = mem->data;
	line_index = 0;
	while (mem->data[line_index] != '\n' && line_index < mem->size)
		line_index++;
	line_ptr = mem->data + line_index;
	line_index = line_ptr - mem->data;
	line = (char *)malloc(sizeof(char) * ((line_index + 1) + 1));
	if (line == NULL)
		return (NULL);
	ft_memmove(line, mem->data, line_index + 1);
	line[line_index + 1] = '\0';
	ft_memmove(mem->data, line_ptr + 1, (mem->size - line_index - 1));
	mem->size = (mem->size - line_index - 1);
	mem->line_cnt -= 1;
	return (line);
}

void *ft_memmove(void *dest, const void *src, size_t n)
{
	unsigned char *d;
	const unsigned char *s;
	size_t	index;

	d = dest;
	s = src;
	if (d < s) {
		index = 0;
		while (index < n)
		{
			d[index] = s[index];
			index++;
		}
	}
	else if (d > s)
	{
		index = n;
		while (index > 0)
		{
			d[index - 1] = s[index - 1];
			index--;
		}
	}
	return (dest);
}

int read_line (int fd, t_gnl_mem *mem)
{
	ssize_t read_len;
	int     index;

	if (reallocate_for_read(mem) == -1)
		return (-1);
	read_len = read(fd, mem->data + mem->size, BUFFER_SIZE);
	if (read_len < 0)
		return (read_len);
	mem->size += read_len;
	index = 0;
	while (index < read_len)
	{
		if (mem->data[mem->size - read_len + index] == '\n')
			mem->line_cnt += 1;
		index++;
	}
	return (read_len);
}

int reallocate_for_read(t_gnl_mem *mem)
{
	char    *tmp;

	if (mem->size + BUFFER_SIZE > mem->max_size) // mem->max_size は最初0の予定。
	{
		mem->max_size += BUFFER_SIZE * ++mem->alloc_cnt;
		tmp = malloc(sizeof(char) * mem->max_size);
		if (tmp == NULL)
			return (-1);
		ft_memmove(tmp, mem->data, mem->size);
		free(mem->data);
		mem->data = tmp;
	}
	return (0);
}
